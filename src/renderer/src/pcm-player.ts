/**
 * Plays a 48kHz s16le stereo PCM stream on the host's own speakers. One instance per
 * stream: the local monitor (the bot's live mix) and the preview bus (GM-only audition)
 * each own a PcmPlayer with its own AudioContext, gain, and output device.
 *
...
export class PcmPlayer {
  ...
  private localVolume = DEFAULT_VOLUME // independent volume for THIS local path (NOT the Discord send level)
  private sinkId = '' // chosen output device ('' = system default)
  private starting = false

  /** `label` only prefixes console diagnostics (e.g. '[monitor]', '[preview]'). */
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (ctx.sampleRate !== 48000) {
        console.warn(
          `[${this.label}] AudioContext came back at ${ctx.sampleRate}Hz, not 48000 — local output ` +
            `will be wrong-pitched. Needs the worklet-resampler fallback (HANDOFF §5.4).`
        )
      } else {
        console.log(`[${this.label}] AudioContext @ 48000Hz (platform resamples to the device)`)
      }
      ...
      node.port.onmessage = (e) => {
        ...
        if (bad > lastBad) {
          console.warn(`[${this.label}] glitch — buffer=${s.fillMs}ms underruns=${s.underruns} overflows=${s.overflows}`)
          lastBad = bad
        }
      }
      this.ctx = ctx
      ...
      console.error(`[${this.label}] failed to start:`, err)
    } finally {
      this.starting = false
    }
  }
  ...
  /**
   * Choose which output device this player plays to ('' = system default).
  ...
  private async applySink(ctx: AudioContext): Promise<void> {
    ...
    try {
      ...
      console.warn(`[${this.label}] setSinkId failed (device gone?) — falling back to default:`, err)
    }
  }
